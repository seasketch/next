  ...
  const progressBar = new cliProgress.SingleBar(
    {
      format: `${filePath
        ...
        .slice(
          -1
        )} | {bar} | {percentage}% | {eta}s || {value}/{total} cells processed`,
    },
        ...
        Papa.parse<{ id: string } & any>(stream, {
          ...
          step: (row: any) => {
            ...
            layer.fields.forEach((field) => {
              if (typeof row.data[field.name] === "boolean") {
                feature.fields.set(
                  field.name,
                  row.data[field.name] === true
                    ? 1
                    : row.data[field.name] === false
                    ? 0
                    : null
                );
              } else {
...
(async () => {
  ...
  const files = readdirSync("./output")
    .filter(
      (file) =>
        file.toString().startsWith("cells-") && file.toString().endsWith(".csv")
    )
    .sort((a, b) => {
  ...
  for (const file of files) {
    await csvToFGB(
      `./output/${file}`,
      `./output/${file.toString().replace(".csv", ".fgb")}`
    );
  }
})();
