...
function default_1(symbol, pixelRatio) {
    var _a, _b;
    const size = utils_1.ptToPx(symbol.size || 13);
    const scale = 2 ** (pixelRatio - 1);
    const padding = 1;
    const width = (size + 1 * 2 + (((_a = symbol.outline) === null || _a === void 0 ? void 0 : _a.width) || 0) * 2) * scale;
    const height = width;
    let canvas = utils_1.createCanvas(width, height);
    var ctx = canvas.getContext("2d");
    ctx.lineWidth =
        utils_1.ptToPx(!!symbol.outline ? symbol.outline.width || 1 : 1) * scale;
    ctx.strokeStyle = !!symbol.outline
        ? utils_1.rgba((_b = symbol.outline) === null || _b === void 0 ? void 0 : _b.color)
        : utils_1.rgba(symbol.color);
    ctx.fillStyle = utils_1.rgba(symbol.color);
    switch (symbol.style) {
        case "esriSMSCircle":
        ...
        case "esriSMSCross":
            ...
            ctx.strokeStyle = utils_1.rgba(symbol.color);
