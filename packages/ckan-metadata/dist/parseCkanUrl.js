...
exports.parseCkanUrl = parseCkanUrl;
exports.localizedDatasetPageUrl = localizedDatasetPageUrl;
...
function originAndPrefix(url, stopAt) {
    const segments = url.pathname.split("/").filter(Boolean);
    const stop = segments.findIndex((segment) => segment.toLowerCase() === stopAt.toLowerCase());
    const prefix = stop === -1 ? segments : segments.slice(0, stop);
    return stripTrailingSlash(`${url.origin}/${prefix.join("/")}`);
}
function parseCkanUrl(input) {
    ...
    if (API_PACKAGE_SHOW.test(url.pathname)) {
        const datasetId = url.searchParams.get("id") || url.searchParams.get("name");
        if (!datasetId) {
            return null;
        }
        const apiRoot = stripTrailingSlash(`${url.origin}${url.pathname.replace(/\/package_show\/?$/i, "")}`);
...
function localizedDatasetPageUrl(parsed, requestedLocale) {
...
//# sourceMappingURL=parseCkanUrl.js.map