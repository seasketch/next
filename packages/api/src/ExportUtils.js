...
function getAnswers(componentName, exportId, componentSettings, answer) {
  if (componentName in components) {
    return components[componentName].getAnswers(componentSettings, exportId, answer);
  } else {
  ...
  for (const element of sortedElements) {
    if (element.isInput) {
      if (element.typeId in components) {
        columns.push(...components[element.typeId].getColumns(element.componentSettings, element.exportId));
      } else {
...
registerComponent("Name", (componentSettings, exportId) => {
...
registerComponent("Consent", (componentSettings, exportId) => {
...
registerComponent("MultipleChoice", (componentSettings, exportId) => {
  return [exportId];
}, (settings, exportId, answer) => {
  return {
    [exportId]: settings.multipleSelect ? answer : Array.isArray(answer) ? answer[0] : void 0
  };
});
registerComponent("ComboBox", (componentSettings, exportId) => {
  return [exportId];
}, (settings, exportId, answer) => {
  return {
...
registerComponent("Matrix", (componentSettings, exportId) => {
...
registerComponent("SpatialAccessPriorityInput", (componentSettings, exportId) => {
