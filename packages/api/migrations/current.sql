...
create or replace function update_eez_clipping_settings(slug text, selections text[], enable_clipping boolean)
  returns project_geography_settings
  language sql
  stable
  security definer
  as $$
    ...
    set eez_selections = selections, enable_eez_clipping = (
      case
        when array_length(selections, 1) is null or selections = '{}' then false
