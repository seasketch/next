--! Previous: sha1:00a45cfca76013eea06ea6d96943eff01061025a
--! Hash: sha1:555da213d419d160a4032e196f0cdf9e0e813660
...
alter table projects drop column if exists data_sources_bucket;
...
insert into data_sources_buckets (name, region, bucket, location) values ('São Paulo', 'sa-east-1', 'geojson-4.seasketch-data.org', ST_GeomFromText('POINT(-47.9 -22.6)', 4326));
...
alter table projects add column data_sources_bucket_id text not null references data_sources_buckets (bucket) default 'geojson-1.seasketch-data.org';
...
create table data_sources (
  ...
  bucket_id text references data_sources_buckets (bucket) on delete cascade,
  object_key uuid,
  byte_length int
);
...
comment on column data_sources.byte_length is 'SEASKETCH_VECTOR sources only. Approximate size of the geojson source';
