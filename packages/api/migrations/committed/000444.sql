--! Previous: sha1:3ea1d1892806e01a290442770213b717925d73f4
--! Hash: sha1:cbb7ae9752adb764f1dd72b52ace9d2fb44175e6
...
comment on function public.create_overlay_data_table_organism_reprocess(integer, jsonb, text, text) is
  'Admin-only. Starts a draft organism-enrichment job. Optional class CSV is uploaded to the returned presigned URL, then submitOverlayDataTableUpload. Writes a clustered copy of data.parquet and organism sidecars under a new upload prefix. Queue wait and running timeout are 15 minutes.';
...
create or replace function public.submit_overlay_data_table_upload(job_id uuid)
...
revoke all on function public.submit_overlay_data_table_upload(uuid) from public;
grant execute on function public.submit_overlay_data_table_upload(uuid) to seasketch_user;
...
comment on function public.complete_overlay_data_table_organism_reprocess(uuid, jsonb, text, text, text) is '@omit';
revoke all on function public.complete_overlay_data_table_organism_reprocess(uuid, jsonb, text, text, text) from public;

-- Rolling-deploy bridge for workers paired with the pre-clustering handler,
-- which writes organism sidecars beside the table's current parquet.
create or replace function public.complete_overlay_data_table_organism_reprocess(
  job_id uuid,
  p_organism jsonb
) returns public.overlay_data_tables
...
create or replace function public.copy_table_of_contents_item_recursive(
  item_id integer,
  copy_data_source boolean,
  append_copy_to_name boolean,
  project_id integer,
  lpath public.ltree,
