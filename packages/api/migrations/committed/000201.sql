--! Previous: sha1:15f180becc2f4e2e4b90da55f165a25a44dcd718
--! Hash: sha1:100d97ce6f2c9dc82339f94f0d8de5e60220292e

-- Enter migration here
alter table data_layers add column if not exists static_id text CONSTRAINT length_check CHECK (char_length(static_id) <= 128);

comment on column data_layers.static_id is 'Used as a stable reference identifier for the layer. In the event that the layer is completely replaced, this ID can be assigned to the new one. Geoprocessing Clients (reports) are an important use case for these IDs, which they use to toggle layers on and off. Map Bookmarks will also use this identifier if present. In both cases, the numeric ID of DataLayers can be used but this is more likely to change.';
