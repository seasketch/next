...
alter table report_card_layers add column if not exists layer_parameters jsonb not null default '{}'::jsonb;
do $$
begin
  if exists (
    select 1 
    ...
    where table_schema = 'public' 
      and table_name = 'report_card_layers' 
      and column_name = 'group_by'
  ) then
    update report_card_layers 
    set layer_parameters = jsonb_build_object('groupBy', group_by)
    where group_by is not null;
  end if;
end $$;
drop index if exists report_card_layers_unique_report_card_id_table_of_contents_item;
alter table report_card_layers drop column if exists group_by;
create index if not exists report_card_layers_unique_configuration on report_card_layers (report_card_id, table_of_contents_item_id, layer_parameters);
...
alter table spatial_metrics add column if not exists parameters jsonb not null default '{}'::jsonb;
...
begin
  if exists (
    ...
    where table_schema = 'public' 
      ...
      and column_name = 'overlay_group_by'
  ) then
    update spatial_metrics set parameters = jsonb_build_object('groupBy', overlay_group_by) where overlay_group_by is not null;
  end if;
end $$;

alter table spatial_metrics drop column if exists included_properties;
alter table spatial_metrics drop column if exists overlay_group_by;

-- create unique constraint on coalese(overlay_source_url, ''), coalese(source_processing_job_dependency, ''), coalesce(subject_fragment_id, ''), coalesce(subject_geography_id, ''), and parameters
create unique index if not exists spatial_metrics_unique_metric on spatial_metrics (coalesce(overlay_source_url, ''), coalesce(source_processing_job_dependency, ''), coalesce(subject_fragment_id, ''), coalesce(subject_geography_id, -999999), parameters);

-- Base function to convert a spatial_metrics row to JSON
CREATE OR REPLACE FUNCTION public.spatial_metric_to_json(sm spatial_metrics) RETURNS jsonb
    LANGUAGE sql STABLE
    AS $$
...
drop type if exists public.report_layer_input cascade;
...
CREATE OR REPLACE FUNCTION public.add_report_card(report_tab_id integer, component_settings jsonb, card_type text, body jsonb, layers public.report_layer_input[]) RETURNS public.report_cards
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
    declare
