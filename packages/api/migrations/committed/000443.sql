--! Previous: sha1:4cc1b78a02aae433890eb949da86ef256050c6bb
--! Hash: sha1:3ea1d1892806e01a290442770213b717925d73f4
...
comment on column overlay_data_tables.description is
  'Optional short description shown in the data table picker. Displayed on at most two lines.';

alter table overlay_data_tables
  drop constraint if exists overlay_data_tables_description_length;

alter table overlay_data_tables
  add constraint overlay_data_tables_description_length
  check (description is null or char_length(description) <= 200);

create or replace function update_overlay_data_table_details(
  table_id integer,
  name text,
  description text
)
...
comment on function update_overlay_data_table_details(integer, text, text) is
  'Admin-only. Updates the display name and optional short description of a draft overlay data table.';

grant execute on function update_overlay_data_table_details(integer, text, text) to seasketch_user;
