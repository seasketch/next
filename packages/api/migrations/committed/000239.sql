--! Previous: sha1:d2c24bb5aaf24b83d77f7cd0c879730cc03cbe20
--! Hash: sha1:47df3314f3f45f97fac756a42c1dfaba57b6016b

-- Enter migration here
alter table basemaps add column if not exists translated_props jsonb not null default '{}'::jsonb;
grant select(translated_props) on basemaps to anon;
...
grant update(translated_props) on table_of_contents_items to seasketch_user;

alter table data_sources add column if not exists translated_props jsonb not null default '{}'::jsonb;
grant select(translated_props) on data_sources to anon;
grant update(translated_props) on data_sources to seasketch_user;
...
alter table sketch_classes add column if not exists translated_props jsonb not null default '{}'::jsonb;
grant select(translated_props) on sketch_classes to anon;
grant update(translated_props) on sketch_classes to seasketch_user;

grant update(translated_props) on forums to seasketch_user;
grant select(translated_props) on forums to seasketch_user;
