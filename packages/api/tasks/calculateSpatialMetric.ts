  ...
  await helpers.withPgClient(async (client) => {
    try {
      const result = await client
        .query(`select get_spatial_metric($1) as metric`, [payload.metricId])
        .catch((e) => {
          throw new Error(`Error getting spatial metric: ${e.message}`);
        });
      ...
      console.error("Error calculating spatial metric", e);
      await client.query(
