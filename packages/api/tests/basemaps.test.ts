...
describe("Basemaps", () => {
  ...
  describe("Basemap Optional Layers", () => {
    ...
    test("Radio groups can be specified to enable only one layer toggled at one time", async () => {
      await projectTransaction(
        pool,
        "public",
        async (conn, projectId, adminId, [user1, user2]) => {
          ...
          const fishId = await conn.oneFirst(
            sql`insert into optional_basemap_layers (basemap_id, layers, name, group_type, group_label) values (${basemapId}, ${sql.array(
              ["layer1"],
              "text"
            )}, 'fish', 'RADIO', 'Biological Data') returning id`
          );
          const squidId = await conn.oneFirst(
            sql`insert into optional_basemap_layers (basemap_id, layers, name, group_type, group_label) values (${basemapId}, ${sql.array(
              ["layer2"],
              "text"
            )}, 'squid', 'RADIO', 'Biological Data') returning id`
          );
          await createSession(conn, user1, true, false, projectId);
          const optionalLayers = await conn.any(
            sql`select * from optional_basemap_layers where basemap_id = ${basemapId}`
          );
          expect(optionalLayers.length).toBe(2);
        }
      );
    });

    test("Radio groups must have a group_label assigned", async () => {
      await projectTransaction(
        pool,
        "public",
        async (conn, projectId, adminId, [user1, user2]) => {
          ...
          expect(
            conn.oneFirst(
              sql`insert into optional_basemap_layers (basemap_id, layers, name, group_type) values (${basemapId}, ${sql.array(
                ["layer1"],
                "text"
              )}, 'fish', 'RADIO') returning id`
            )
          ).rejects.toThrow(/group_label/);
        }
      );
    });

    test("Admins can use the updateBasemapLayerRadioGroupLabel function to rename radio groups", async () => {
      await projectTransaction(
        pool,
        "public",
        async (conn, projectId, adminId, [user1, user2]) => {
          ...
          const unrelatedId = await conn.oneFirst(
            sql`insert into optional_basemap_layers (basemap_id, layers, name) values (${basemapId}, ${sql.array(
              ["layer2"],
              "text"
            )}, 'marxan') returning id`
          ...
          await conn.any(
            sql`select update_basemap_layer_radio_group_label(${basemapId}, 'Biological Data', 'Organisms')`
          );
          const updated = await conn.any(
            sql`select * from optional_basemap_layers where group_label = 'Organisms'`
          );
