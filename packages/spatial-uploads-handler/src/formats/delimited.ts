...
const SEPARATOR_BY_DELIMITER: Record<
  DelimitedUploadProcessingOptions["delimiter"],
  string
> = {
  ",": "COMMA",
  "\t": "TAB",
 ...
 * reference.
 */
export async function convertDelimitedToGeoJSON(
  logger: Logger,
  inputPath: string,
  outputPath: string,
  options: DelimitedUploadProcessingOptions,
): Promise<void> {
  if (options.crs && options.crs !== "EPSG:4326") {
    throw new Error(
      "Only WGS 84 (EPSG:4326) decimal degree coordinates are supported for delimited uploads.",
    );
  }
  ...
  if (options.geometryMode === "wkt") {
    if (!options.geometryField) {
      throw new Error(
        "processingOptions.geometryField is required when geometryMode is 'wkt'",
      );
    }
    ...
    if (!options.xField || !options.yField) {
      throw new Error(
        "processingOptions.xField and yField are required when geometryMode is 'point_xy'",
      );
    }
    openOptions.push(
      "-oo",
      `X_POSSIBLE_NAMES=${options.xField}`,
      "-oo",
      `Y_POSSIBLE_NAMES=${options.yField}`,
    );
  }
  ...
  await logger.exec(
    ...
    "Problem converting delimited text file to spatial data. Double check the selected columns and coordinate reference system.",
    2 / 30,
  );
}
