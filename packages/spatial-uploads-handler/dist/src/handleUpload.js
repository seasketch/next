...
requestingUser, skipLoggingProgress) {
    ...
    let stats;
    let columnIntelligence;
    const columnIntelligenceUploadedFilename = process.env.COLUMN_INTELLIGENCE_DISABLE_UPLOAD_PREFETCH === "true"
        ? undefined
        : `${originalName}${ext}`;
    ...
    try {
        if (isTif || ext === ".nc") {
            const rasterOut = await (0, processRasterUpload_1.processRasterUpload)({
                logger,
            ...
            const vectorOut = await (0, processVectorUpload_1.processVectorUpload)({
                logger,
                path: workingFilePath,
                outputs,
                updateProgress,
