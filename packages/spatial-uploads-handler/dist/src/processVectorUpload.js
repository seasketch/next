...
async function processVectorUpload(options) {
    ...
    if (isZip || isRar) {
        ...
        // // Unzip the file
        if (isZip) {
            await logger.exec(["unzip", ["-o", workingFilePath, "-d", workingDirectory]], "Problem unzipping file", 1 / 30);
        }
        else {
            await logger.exec(["unrar", ["x", "-op" + workingDirectory, workingFilePath]], "Problem extracting .rar file", 1 / 30);
        }
        ...
        ], "Problem finding shapefile in zip archive", 1 / 30);
        ...
        ], "Problem finding projection file (.prj) in zip archive", 1 / 30);
        if (!projFile) {
        ...
        const { ext } = (0, path_1.parse)(workingFilePath);
        ...
        ], "Problem finding metadata files in zip archive", 1 / 30);
        try {
            if (xmlPaths) {
                const paths = xmlPaths.trim().split("\n");
                for (const xmlPath of paths) {
                    try {
                        const data = (0, fs_1.readFileSync)(xmlPath.trim(), "utf8");
                        const parsedMetadata = await (0, metadata_parser_1.metadataToProseMirror)(data);
                        if (parsedMetadata && Object.keys(parsedMetadata).length > 0) {
                            metadata = parsedMetadata;
                            outputs.push({
                                type: "XMLMetadata",
                                filename: xmlPath.split("/").pop(),
                                remote: `${process.env.RESOURCES_REMOTE}/${baseKey}/${jobId}/${xmlPath.split("/").pop()}`,
                                local: xmlPath,
                                size: (0, fs_1.statSync)(xmlPath).size,
                                url: `${process.env.UPLOADS_BASE_URL}/${baseKey}/${jobId}/${xmlPath.split("/").pop()}`,
                            });
                            break;
                        }
                    }
    ...
    // of supported types
    const ogrInfo = await logger.exec(["ogrinfo", ["-al", "-so", workingFilePath]], ext === ".shp"
        ? "Could not read file. Shapefiles should be uploaded as a zip archive with related sidecar files"
        : "Could not run ogrinfo on file", 1 / 30);
    ...
    const originalOutput = {
        ...
        size: (0, fs_1.statSync)(originalFilePath).size,
    ...
    let normalizedVectorFileSize = (0, fs_1.statSync)(workingFilePath).size;
    if (!isCorrectProjection || type !== "FlatGeobuf") {
        // Convert vector to a normalized FlatGeobuf file for long-term archiving
        // and for tiling.
        normalizedVectorPath = (0, path_1.join)(workingDirectory, jobId + ".fgb");
        ...
        ], "Problem converting to FlatGeobuf", 2 / 30);
        // Save normalized vector to the list of outputs
        normalizedVectorFileSize = (0, fs_1.statSync)(normalizedVectorPath).size;
        outputs.push({
            type: "FlatGeobuf",
            filename: `${jobId}.fgb`,
    ...
    if (underThreshold) {
        ...
        ], "Problem converting to GeoJSON", 15 / 30);
        ...
        outputs.push({
            ...
            size: (0, fs_1.statSync)(geojsonPath).size,
    ...
    if (normalizedVectorFileSize >= handleUpload_1.MVT_THRESHOLD) {
        ...
        const fioInfo = await logger.exec(["fio", ["info", normalizedVectorPath]], "Problem detecting numeric properties using fiona.", 1 / 30);
        const fioData = JSON.parse(fioInfo);
        const schema = ((_a = fioData === null || fioData === void 0 ? void 0 : fioData.schema) === null || _a === void 0 ? void 0 : _a.properties) || {};
         ...
         */
        const mvtPath = (0, path_1.join)(workingDirectory, jobId + ".mbtiles");
        ...
        ], "Tippecanoe failed", 12 / 30);
        // TODO: at some point a newer tippecanoe can be used to save directly to
        // pmtiles
        await logger.exec([`pmtiles`, ["convert", mvtPath, pmtilesPath]], "PMTiles conversion failed", 3 / 30);
        outputs.push({
            type: "PMTiles",
