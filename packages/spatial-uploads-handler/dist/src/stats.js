...
exports.equalIntervalBuckets = equalIntervalBuckets;
exports.quantileBuckets = quantileBuckets;
exports.stdDevBuckets = stdDevBuckets;
exports.naturalBreaksBuckets = naturalBreaksBuckets;
exports.geometricIntervalBuckets = geometricIntervalBuckets;
exports.getRepresentativeColors = getRepresentativeColors;
...
function stdDevBuckets(data, numBuckets, mean, stdDev, min, max, fraction = false) {
    min = min !== undefined ? min : Math.min(...data);
    max = max !== undefined ? max : Math.max(...data);
...
function naturalBreaksBuckets(data, numBuckets, 
/** Number of unique values in data */
uniqueCount, min, max, downsampleTo, fraction = false) {
    min = min !== undefined ? min : Math.min(...data);
    max = max !== undefined ? max : Math.max(...data);
    ...
    const clusters = (0, simple_statistics_1.ckmeans)(downsampled, numBuckets);
    let breaks = [];
...
function geometricIntervalBuckets(data, numBuckets, min, max, fraction = false) {
    min = min !== undefined ? min : Math.min(...data);
    max = max !== undefined ? max : Math.max(...data);
    const logMax = Math.log(max) / Math.LN10;
    let logMin = Math.log(min) / Math.LN10;
    if (logMin < 0) {
        logMin = 0;
    }
    const interval = (logMax - logMin) / numBuckets;
    ...
    const breaks = initialBuckets.map((el) => el === 0 ? 0 : Math.pow(10, el));
    return breaksToBuckets(max, breaks, data, fraction);
}
...
function getQuantiles(population, numBuckets, includeDuplicates = false) {
    // use simple-statistics quantile() function to get quantiles for numBuckets
    const quantiles = [];
    for (let i = 1; i < numBuckets; i++) {
        quantiles.push((0, simple_statistics_1.quantile)(population, i / numBuckets));
...
function getStdDevBreaks(numBins, min, max, mean, stdDev) {
...
function getRepresentativeColors(pixels, numColors, downsampleTo) {
    let flattenedPixels = pixels.map((pixel) => rgbToNumber(pixel[0], pixel[1], pixel[2]));
    flattenedPixels.sort((a, b) => a - b);
    if (downsampleTo) {
        flattenedPixels = downsampleArray(flattenedPixels, downsampleTo);
    }
    const clusters = (0, simple_statistics_1.ckmeans)(flattenedPixels, numColors);
