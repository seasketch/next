...
const processUpload = async (event) => {
    ...
    try {
        const outputs = await (0, handleUpload_1.default)(event.taskId, event.objectKey, event.suffix, event.requestingUser, event.skipLoggingProgress, event.enableAiDataAnalyst, event.processingOptions);
        return {
            ...outputs,
            log: s3LogPath,
        };
