...
> = (props) => {
  ...
  // Why do this twice?
  useEffect(() => {
    if (manager && basemaps && mapOverlayState.styleHash.length > 0) {
      manager.getComputedStyle().then((style) => setMiniMapStyle(style.style));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [manager, mapOverlayState.styleHash]);
        ...
        <BasemapContext.Provider value={{ ...basemapState, basemaps }}>
          <MapOverlayContext.Provider value={mapOverlayState}>
            <LegendsContext.Provider value={legendsState}>
              <MapUIProvider preferencesKey="spatial-access-priority">
                <>
                  <AnimatePresence
                    initial={false}
                              ...
                              <React.Fragment key={feature.id}>
                                ...
                                >
                                  <input
                                    ...
                                    step={5}
                                    value={
                                      feature.properties[priorityElementId!]
                                    }
                                    onChange={(e) => {
                      ...
                      {(props.stage === STAGES.SHAPE_EDITOR ||
                        ...
                        <div className="py-5 space-y-2">
                          ...
                          <div className="space-x-2 rtl:space-x-reverse flex items-center">
                            ...
                            {(langContext.supportedLanguages || []).length >
                              0 && (
                              <LanguageSelector
                        ...
                        {props.stage !== STAGES.MOBILE_EDIT_PROPERTIES && (
                          <DigitizingTools
                              ...
                              style.isSmall && !geometryEditingState?.isNew ? (
                                <>
                                  <Button
                                    className="pointer-events-auto"
                                    label={
                                      <Trans ns="surveys">Back to List</Trans>
                                    }
                                    onClick={() => {
                                      props.onRequestStageChange(
                                        STAGES.LIST_SHAPES
                            ...
                            onRequestResetFeature={() => {
                              if (!selection) {
                                throw new Error(
                                  "No selection to perform feature reset"
                                );
                              }
                              if (geometryEditingState?.isNew) {
                                throw new Error(
