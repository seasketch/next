...
function buildHistogramExpression(expression: Expression): Expression {
  ...
  return /interpolate/.test(fnType)
    ? [expression[0], expression[1], ["get", "value"], ...expression.slice(3)]
...
function getRasterInfo(source?: {
  geostats?: RasterInfo | unknown;
...
export const RasterValuesHistogram: ReportWidget<
  RasterValuesHistogramSettings
> = ({ metrics, componentSettings, sources, loading }) => {
  ...
  const bandStats = useMemo(() => {
    const fragmentMetrics = metrics.filter(
      (m) =>
        subjectIsFragment(m.subject) && m.type === "raster_stats" && m.value
  ...
  const labelDomain = useMemo(() => getLabelDomain(baseBuckets), [baseBuckets]);
  ...
  const countFormatter = useMemo(() => new Intl.NumberFormat(undefined), []);
                      ...
                      <div className="space-y-0.5">
                        ...
                        {bounds.min === bounds.max ? (
                          <div className="flex justify-between gap-2">
                            ...
                            <span>
                              {`>=`} {formatValueWithUnit(bounds.min)}
                            </span>
                          </div>
                        ) : (
                          <>
          ...
          <LabeledDropdown
            ...
            onChange={(value) =>
              onUpdate({
                componentSettings: {
                  ...componentSettings,
                  labelDensity: value as "none" | "less" | "default" | "more",
                },
              })
            }
          />
