...
- **Zero**, **No data**, **Not surveyed**. A replicate surveyed for a subject that saw none is **zero** when the within-replicate calculation is sum. A mean, min, or max of nothing is **left out** ("no values"). A feature with no replicates left after survey filters is **no data**. A replicate not surveyed for the subject is **not surveyed** and is left out.
