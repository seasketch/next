...
const createBasemaps = (id, token, name) => {
  ...
  body.append(
    "map",
    JSON.stringify({ 1: ["variables.input.basemap.thumbnail"] })
  );
  body.append("1", file);
  const fetchResponse = fetch("http://localhost:3857/graphql", {
    method: "POST",
    headers: {
  ...
  describe.only("User survey flow", () => {
    beforeEach(() => {
      cy.intercept("http://localhost:3857/graphql", (req) => {
        if (
          ...
          req.body.operationName === "CreateResponse"
        ) {
        ...
        if (
          req.body.operationName &&
          req.body.operationName === "GetBasemapsAndRegion"
        ) {
          req.alias = "getBasemaps";
        }
        if (
          req.body.operationName &&
          req.body.operationName === "ProjectRegion"
        ) {
          req.alias = "getProjectRegion";
        }
        if (
          req.body.operationName &&
          req.body.operationName === "ProjectMetadata"
        ) {
          ...
          cy.wrap(projectId)
            ...
            .then(() => {
              cy.createSurvey(
                ...
                cy.wrap(resp.makeSurvey.survey.form.formElements).as(
                  "formElements"
                );
                ...
                cy.get("@formId").then((formId: any) => {
                  cy.deleteFormElements(formId, access_token).then((resp) => {
                    ...
                    resp.deleteFormElement.query.form.formElements.forEach(
                      (t) => {
                        elementsToUpdate.push(t);
                      }
                    );
                    cy.updateFormElements(
                      elementsToUpdate,
                      "Maldives",
                      access_token,
                      formId
                    );
                    cy.createFormElements(
                      formId,
                      "Maldives",
                      access_token
                    ).then((resp) => {
                      const SAPFormId = formId + 1;
                      cy.createSAPElements(
                        SAPFormId,
                        "Maldives",
                        access_token
                      ).then((resp) => {
                        ...
                        let SAPElements =
                          resp.createFormElement.query.form.formElements;
                        cy.setLocalStorage(
                          "SAPElements",
                          JSON.stringify(SAPElements)
                        );
                      });
                      const formElements =
                        resp.createFormElement.query.form.formElements;
                      ...
                      formElements.forEach((t) => {
                        if (
                          t.body.content[0].content[0].text ===
                          "If you are representing a guesthouse, please provide the name of your establishment:"
                        ) {
                          updateSubToIdElements.push(t);
                        } else if (
                          t.body.content[0].content[0].text ===
                          "Please indicate how many people are reflected in this response"
                        ) {
                          updateSubToIdElements.push(t);
                        } else if (
                          t.body.content[0].content[0].text ===
                          "Are you a part-time or full-time fisher?"
                        ) {
                          updateSubToIdElements.push(t);
                        } else if (
                          t.body.content[0].content[0].text ===
                          "Please provide the name or the number of the vessel you fish on"
                        ) {
                          updateSubToIdElements.push(t);
                        }
                      });
                      cy.get("@sapId").then((id: any) => {
                        cy.updateSubordinateToId(
                          id,
                          updateSubToIdElements,
                          formId,
                          access_token
                        );
                      });
                        ...
                        if (baseId === 0) {
                          for (let i = 0; i < formElements.length; i++) {
                            if (
                              ...
                              formElements[i].body.content[0].content[0]
                                .text === "Which Atoll do you reside on?"
                            ) {
                      ...
                      const updateComponentSettings = (formElements) => {
                        ...
                        formElements.forEach((t) => {
                          ...
                          if (
                            exportIds[t.exportId] &&
                            t.exportId !== "tuna_gear"
                          ) {
                        ...
                        cy.get("@sapId").then((id: any) => {
                          ...
                          cy.updateComponentSettings(
                            id,
                            referenceElements,
                            access_token,
                            formId
                          ).then((resp) => {
                            let values: object = Object.values(
                              resp.updateFormElement.formElement
                                .componentSettings.childVisibilitySettings
                            );
                            expect(values[0].enabled).to.eq(true) &&
                              expect(values[0].sectors.toString()).to.eq(
                                "Fisheries - Commercial, Tuna"
                              );
                      ...
                      cy.get("@createFormLogicResponse").then((resp: any) => {
                        settingsElements.push(
                          resp.createFormLogicCondition.query.form.formElements
                        );
                      });
                      cy.get("@SAPResponse").then((resp: any) => {
                        let settingsAry = settingsElements[0].concat(
                          resp.createFormElement.query.form.formElements
                        );
    ...
    devices.forEach((device) => {
      ...
      it(`Cannot advance until island selection is made - ${device}`, () => {
        ...
        cy.getLocalStorage("slug").then((slug) => {
          cy.getLocalStorage("surveyId").then((id) => {
            cy.url().should(
              "eq",
              Cypress.config().baseUrl + `/${slug}/surveys/${id}/7/`
            );
          });
        });
        cy.contains("Which Atoll do you reside on?").should("not.exist");
        cy.contains("Which island of N atoll do you reside on?");
        //cy.percySnapshot(`${Cypress.currentTest.titlePath}`);
        cy.contains("Lhohi");
        cy.get('[title = "Next Question"]').should(
          "have.class",
          "pointer-events-none"
        );
      ...
      it(`Can draw a polygon - Fisheries - Commercial, Tuna - ${device}`, () => {
        cy.viewport(device);
        //cy.percySnapshot(`${Cypress.currentTest.titlePath}`);
        cy.get('[title = "Fisheries - Commercial, Tuna"]').should("not.exist");
        cy.get("button").contains("Next").should("not.exist");
        cy.get("h4")
          .contains("Fisheries - Commercial, Tuna")
          .should("exist")
          .and("be.visible");
        ...
        if (device === "iphone-x") {
          ...
          waitOnMapbox(9);
          cy.get("span.mapboxgl-ctrl-icon").should("be.visible");
          cy.get("div.MapPicker").should("be.visible");
          drawPolygon();
        } else {
          if (device === "macbook-15") {
            ...
            waitOnMapbox(8);
            cy.get("span.mapboxgl-ctrl-icon").should("be.visible");
            cy.get("div.MapPicker").and("be.visible");
            drawPolygon();
          } else {
            //ipad-2
            cy.contains("Fisheries").should("be.visible");
            cy.get("span.mapboxgl-ctrl-icon").should("be.visible");
            cy.get("div.MapPicker").should("exist").and("be.visible");
      ...
      it(`Can view basemap selector - ${device}`, () => {
        cy.viewport(device);
        cy.get('img[alt="Satellite map preview"]').click({ force: true });
        let values = [
          "Reset view",
          "Focus on location",
          "Show scale bar",
        ...
        cy.get('img[alt="Satellite basemap"]').should("be.visible");
        cy.get('img[alt="Maldives Light basemap"]').should("be.visible");
      });
      ...
      it(`Renders the correct basemap - ${device}`, () => {
        ...
        cy.contains("Satellite").should("not.have.class", "font-semibold");
        //cy.percySnapshot(`${Cypress.currentTest.titlePath}`);
      });
      it(`Can select different basemap - ${device}`, () => {
        cy.viewport(device);
        cy.contains("Satellite").as("satelliteBasemap");
        cy.get("@satelliteBasemap")
          .then(($btn) => {
            {
              $btn.trigger("click");
            }
        ...
        cy.contains("Maldives Light").should("not.have.class", "font-semibold");
        //cy.percySnapshot(`${Cypress.currentTest.titlePath}`);
      });
      it(`Shows option to focus on location - ${device}`, () => {
        ...
        cy.window()
          .its("mapContext.map.transform._center")
          .as("centerCoords")
          .then((center) => {
      ...
      it(`Renders sector specific attributes - Fisheries - Commercial, Tuna - ${device}`, () => {
        ...
        cy.get("h1").contains("Area Name").should("exist");
        cy.get('[name="FeatureName"]')
          .scrollIntoView()
          .clear({ force: true })
          .type("Yellowfin tuna fishing area.");
        cy.get('[name="FeatureName"]').should(
          "have.value",
          "Yellowfin tuna fishing area."
        );
      ...
      it(`Can set area importance using SAP range slider - ${device}`, () => {
        ...
        cy.get("@range").then(($range) => {
          ...
          //@ts-ignore
          range.dispatchEvent(
            new Event("change", { value: 15, bubbles: true })
          );
        });
        cy.get("@range").should("have.value", 15);
        cy.get("button")
          .contains("Save")
          .then(($save) => {
            {
        ...
        cy.get(".SAPRangeMini").and("be.visible").and("have.value", 15);
        //cy.percySnapshot(`${Cypress.currentTest.titlePath}`);
      });
      it(`Can finish sector - Fisheries - Commercial, Tuna - ${device}`, () => {
        ...
        if (device === "iphone-x") {
          cy.get("button").contains("New Shape").should("be.visible");
          cy.contains("View Map").should("be.visible");
        }
        cy.get("button")
          .contains("Finish Sector")
          .as("finishSector")
          .should("be.visible")
          .then(($el) => {
            {
        ...
        cy.get("h1").contains("Your sectors").should("exist").and("be.visible");
        cy.get(".space-y-2").should("exist").and("be.visible");
        cy.get("button")
          .contains("Next sector")
          .as("nextSectorBtn")
          .should("exist")
          .and("be.visible");
      ...
      it(`Can draw a polygon - Fisheries - Commercial, Non-Tuna Species - ${device}`, () => {
        ...
        if (device === "iphone-x" || device === "iphone-5") {
          ...
          cy.get('[data-cy="button-begin"]').should("not.be.visible");
          cy.get("p")
            .contains("Click on the map")
            .should("exist")
            .and("be.visible");
          drawPolygon();
          cy.get('[data-cy="button-done"]')
      ...
      it(`Renders sector specific attributes - Fisheries - Commercial, Non-Tuna Species - ${device}`, () => {
        cy.viewport(device);
        cy.contains("Area Name").should("exist");
        ...
        cy.get('[name="FeatureName"]').should(
          "have.value",
          "Sea cucumber fishing area."
        );
        cy.contains("What type of gear");
        cy.contains("What type of species");
        cy.get('[title="Pole and Line"]').should("not.exist");
        cy.get('[title="Pole and Line"]').should("not.exist");
        cy.get('[title="Yellowfin"]').should("not.exist");
        cy.get('[title="Sea cucumber"]').click();
        cy.get('[title="Jigging"]').click();
        cy.get('[style="max-height: 60vh;"] > .w-full').type(
          "Sea cucumber love this spot!"
        );
      ...
      it(`Errors when invalid polygon is drawn - Fisheries - Commercial, Non-Tuna Species - ${device}`, () => {
        ...
        cy.get("@newShape").should("not.exist");
        cy.get('[role="progressbar"]').should("not.exist");
        drawInvalidPolygon();
      ...
      it(`Can draw second shape - Fisheries - Commercial, Non-Tuna Species - ${device}`, () => {
        ...
        if (device === "macbook-15" || device === "ipad-2") {
          ...
          cy.get("h4").contains("Fisheries - Recreational").should("not.exist");
          cy.contains("Area Name").should("exist").and("be.visible");
      ...
      it(`Renders sector specific attributes for second shape - Fisheries - Commercial, Non-Tuna Species - ${device}`, () => {
        ...
        cy.get('[title="Pole and Line"]').should("not.exist");
        cy.get('[title="Yellowfin"]').should("not.exist");
        cy.get("button").contains("Handline").should("be.visible").click();
        cy.get("button").contains("Reef fish").should("be.visible").click();
        cy.get('[style="max-height: 60vh;"] > .w-full').type(
          "Bountiful reef fishing."
        );
        cy.contains("Save").click();
      });
      ...
      it(`Shows completed sectors - Fisheries - Commercial, Non-Tuna Species - ${device}`, () => {
        ...
        cy.get("button")
          ...
          .parent()
          .then(($btn) => {
            expect($btn.css("background")).to.include(
              "rgba(0, 0, 0, 0) linear-gradient(rgb(62, 188, 181), rgb(39, 160, 153))"
            );
          });
        //completed sector
        cy.get("button")
          .contains("Fisheries - Commercial, Non-Tuna Species")
          .parent()
          .then(($btn) => {
            expect($btn.css("background")).to.equal(
              "rgba(0, 0, 0, 0) linear-gradient(rgb(62, 188, 181), rgb(39, 160, 153)) repeat scroll 0% 0% / auto padding-box border-box"
            );
          });
      ...
      it(`Can input number of people reflected in response - ${device}`, () => {
        cy.viewport(device);
        checkForNavAndLang();
        cy.get('[data-question="yes"]')
          .contains(
            "Please indicate how many people are reflected in this response"
          )
          .should("exist")
          .and("be.visible");
        cy.get('input[type="number"]').as("numberInput").should("exist");
        ...
        cy.get('button[title="Subtract"]').click().click();
        cy.get("@numberInput").then(($input) => {
      ...
      it(`Can input name or number of vessel - ${device}`, () => {
        cy.viewport(device);
        checkForNavAndLang();
        cy.contains("vessel").should("exist").and("be.visible");
        cy.log("Skipping question without inputting value");
        cy.get("input").as("vesselInput").should("be.empty");
        ...
        cy.getLocalStorage("slug").then((slug) => {
          cy.getLocalStorage("surveyId").then((id) => {
            cy.url().should(
              "eq",
              Cypress.config().baseUrl + `/${slug}/surveys/${id}/28/`
            );
          });
        });
        cy.contains("willing").should("exist").and("be.visible");
        cy.get('a[title="Previous Question"]').click();
        cy.contains("vessel").should("be.visible").and("exist");
      ...
      it(`Can answer additional questions - ${device}`, () => {
        cy.viewport(device);
        checkForNavAndLang();
        cy.contains("additional").should("exist").and("be.visible");
        cy.get('[title="Yes"]').should("exist").and("be.visible").click();
      });
      it(`Can input age - ${device}`, () => {
        ...
        cy.getLocalStorage("slug").then((slug) => {
          cy.getLocalStorage("surveyId").then((id) => {
            cy.url().should(
              "eq",
              Cypress.config().baseUrl + `/${slug}/surveys/${id}/29/`
            );
          });
        });
        cy.contains("willing").should("not.exist");
        cy.contains("Your age").should("exist").and("be.visible");
      ...
      it(`Can add comments - ${device}`, () => {
        ...
        cy.contains("General Comments").should("not.exist");
      });
      it(`Records the correct response - ${device} `, () => {
        cy.wait("@createResponse").then((req) => {
          expect(req.response.statusCode).to.eq(200);
          const surveyResponseId =
            req.response.body.data.createSurveyResponse.surveyResponse.id;
            ...
            cy.getSurveyResponse(surveyResponseId, token).then((resp) => {
              ...
              sketchIds.forEach((id) => {
                cy.getSketch(id, token).then((sketch) => {
                  expect(sketch.sketch.userGeom.geojson.coordinates[0]).to.not
                    .be.empty;
                });
              });
              ...
              expect(responseAry[4].sectors[0]).to.eq(
                "Fisheries - Commercial, Tuna"
              );
              expect(responseAry[4].sectors[1]).to.eq(
                "Fisheries - Commercial, Non-Tuna Species"
              );
        ...
        if (device === "macbook-15" || device === "ipad-2") {
          ...
          cy.get("button")
            .contains("Submit Another Response")
            .should("exist")
            .click();
        }
      });
