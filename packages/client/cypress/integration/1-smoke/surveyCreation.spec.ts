  ...
  describe.only('User survey flow', () => {
    ...
    devices.forEach((device) => {
      it(`Can visit the survey - ${device}`, () => {
        ...
        cy.get('.select-none').should('be.visible')
        ...
        //cy.get('@switches').then(($switch) => {
        //  console.log($switch)
        //})
       
        
        //    //expect ($switch.parent()).to.haveOwnProperty('aria-checked', false)
...
//
    ...
    devices.forEach((device) => {
      it(`Can input email address or can skip question - ${device}`, () => {
        ...
        cy.get('button').contains('Next')
          .should('not.exist')
        cy.get('button').contains('Skip Question').as('skipQuestion')
          .should('be.visible')
        cy.get("input").as('emailInput')
          .should('be.visible')
          .type("test_user_1@seasketch.org")
        cy.get('button').contains('Next')
          .should('be.visible')
        cy.get('@emailInput')
          .should('have.value', "test_user_1@seasketch.org" )

          .clear()
      });
    })
    it('Advances to atoll selection', () => {
      cy.get("input")
        .should('be.visible')
        .type("test_user_1@seasketch.org")
  ...
  ////}
    ...
    devices.forEach((device) => {
      it(`Can choose island - ${device}`, () => {
        cy.viewport(device)
      cy.contains("Which island of N atoll do you reside on?").scrollIntoView()
        .should('be.visible')
      cy.contains('Kudafari').scrollIntoView()
        .should('be.visible')
      cy.get('[title = "Next Question"]')
        .should('have.class', "pointer-events-none")
      })
    })
    it("Advances to sector selection", () => {
    ...
    //devices.forEach((device) => {
    //it(`Renders SAP Page - Fisheries - Commercial, Tuna - ${device}`, () => {
