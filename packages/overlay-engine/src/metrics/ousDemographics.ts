...
import { CandidateFeature, ContainerIndex } from "../utils/containerIndex";
...
/**
 * Interim per-respondent record. `representedInSector` is tracked unclamped
 * (max across the respondent's shapes seen so far) and clamped to
 * `participants` when results are finalized.
 */
type RespondentAccumulatorEntry = {
  maxRepresentedInSector: number;
  participants: number;
};

type GroupedRespondentAccumulator = Map<
  string,
  Map<string, RespondentAccumulatorEntry>
>;

function addShapeToAccumulator(
  accumulator: GroupedRespondentAccumulator,
  groupKey: string,
  responseId: string,
  ...
  if (!existing) {
    ...
    existing.maxRepresentedInSector = Math.max(
      existing.maxRepresentedInSector,
      representedInSector,
    );
...
function clampedRepresentedInSector(
  entry: RespondentAccumulatorEntry,
): number {
 ...
 */
export class OusDemographicsAggregator {
  private groupBy: string;
  /** Respondents with >=1 intersecting shape, per group + rollup. */
  private within: GroupedRespondentAccumulator = new Map();
  /** All respondents in the dataset, per group + rollup. */
  private totals: GroupedRespondentAccumulator = new Map();
  ...
  constructor(groupBy: string = OUS_DEMOGRAPHICS_DEFAULT_GROUP_BY) {
    if (groupBy === OUS_DEMOGRAPHICS_ROLLUP_KEY) {
      throw new Error(
        `"${OUS_DEMOGRAPHICS_ROLLUP_KEY}" is a reserved rollup key and cannot be used as a groupBy column for ous_demographics`,
      );
    }
  ...
  addFeature(
    ...
    const responseId =
      responseIdRaw === null || responseIdRaw === undefined
        ? ""
  ...
  result(): OusDemographicsMetricValue {
    if (this.featureCount > 0) {
      ...
      if (missing.length > 0) {
        throw new Error(
          `ous_demographics requires columns response_id, participants, represented_in_sector, and the groupBy column ("${this.groupBy}"). Missing from source: ${missing.join(", ")}`,
        );
      }
      if (this.skippedFeatureCount === this.featureCount) {
        throw new Error(
          `ous_demographics could not read response_id, participants, and represented_in_sector values from any of the ${this.featureCount} features in the source`,
        );
      }
    }

    const value: OusDemographicsMetricValue = { groups: {}, totals: {} };
    for (const [groupKey, respondents] of this.within) {
      const group: OusDemographicsMetricValue["groups"][string] = {};
 ...
 */
export async function calculateOusDemographics(
  subjectFeature: Feature<Polygon | MultiPolygon>,
  source: FlatGeobufSource<Feature<Geometry>>,
  options?: {
  ...
  const groupBy = options?.groupBy || OUS_DEMOGRAPHICS_DEFAULT_GROUP_BY;
  const aggregator = new OusDemographicsAggregator(groupBy);
  const containerIndex = new ContainerIndex(subjectFeature);

  helpers.log(
    `Calculating ous_demographics (groupBy=${groupBy}) with a full scan of the source`,
  );

  for await (const feature of source.getFeaturesAsync(source.bounds)) {
    const classification = containerIndex.classify(
  ...
  if (aggregator.skippedFeatureCount > 0) {
    helpers.log(
      `ous_demographics skipped ${aggregator.skippedFeatureCount} of ${aggregator.featureCount} features missing response_id, participants, or represented_in_sector values`,
    );
  }
