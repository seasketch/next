...
class OverlayEngineBatchProcessor {
    ...
    constructor(operation, maxBatchSize, subjectFeature, intersectionSource, differenceSources, helpers, groupBy, pool, includedProperties, resultsLimit, overlappingFeatures, subjectIsBuffered, overlayOverlapOptions) {
        /**
     ...
     */
    collectsOverlayOverlapEntries() {
        return (this.isOverlayAreaOperation() && this.overlayOverlapOptions !== undefined);
    }
    resetBatchData() {
        this.batchData = {
    ...
    mergeOverlayBatchResults(batchResults) {
        ...
        for (const batchData of batchResults) {
            ...
            for (const classKey in overlayBatchData) {
                ...
                if (!(0, metrics_1.isOverlayAreaClassKey)(classKey)) {
                    continue;
                }
                ...
                results[classKey] = results[classKey] + amount;
            }
        }
    }
    /**
     ...
     */
    finalizeOverlayOverlapMetadata() {
        ...
        for (const entry of this.overlayFeatureEntries) {
            ...
            const partFullyCovered = Math.abs(entry.featureArea - entry.clippedArea) < 1e-9;
        ...
        const allClassKeys = new Set([
            ...byClass.keys(),
            ...Object.keys(results).filter(metrics_1.isOverlayAreaClassKey),
        ]);
        const flat = [];
        ...
        const truncated = flat.length > metrics_1.MAX_OVERLAY_AREA_OVERLAP_ENTRIES;
        const kept = truncated
            ? flat.slice(0, metrics_1.MAX_OVERLAY_AREA_OVERLAP_ENTRIES)
            : flat;
        const keptByClass = new Map();
        ...
        for (const classKey of Object.keys(classes)) {
            ...
            for (const e of list) {
                ...
                const fa = Math.abs(e.featureArea - e.area) < 1e-9 ? 0 : e.featureArea;
                featureArea.push(fa);
                if (fa !== 0) {
                    anyPartial = true;
                }
    ...
    mergeColumnValuesBatchResults(batchResults) {
        ...
        const includeEntries = Boolean(this.includedProperties && this.includedProperties.length > 0);
        for (const classKey in results) {
            columnStats[classKey] = {};
            for (const attr in results[classKey]) {
                columnStats[classKey][attr] = calculateColumnValueStats(results[classKey][attr], includeEntries, this.subjectIsBuffered);
            }
        }
    ...
    addColumnValuesFeatureToResults(feature) {
        const results = this.getColumnValuesResults();
        (0, clipBatch_1.addColumnValuesToResults)(results, feature, this.groupBy, this.includedProperties);
    }
    addOverlayFeatureToTotals(feature) {
        ...
        if (this.collectsOverlayOverlapEntries()) {
            ...
            let inCollar = true;
            try {
                inCollar = (0, boolean_intersects_1.default)(feature, collar);
            }
            catch {
                inCollar = true;
            }
 ...
 */
function calculateColumnValueStats(values, includeEntries, includeOffsets) {
    ...
    const stats = typeof firstValue === "string" || typeof firstValue === "boolean"
        ? (0, metrics_1.stringOrBooleanColumnStatsFromEntries)(entries)
        : (0, metrics_1.numberColumnStatsFromEntries)(entries);
    if (includeEntries) {
        const capped = (0, metrics_1.capColumnValueEntries)(entries);
        if (capped.entries) {
