...
export ADMIN_DATABASE_URL="postgres://$PGUSER:$(node --eval "console.log(encodeURIComponent(process.env.PGPASSWORD))")@$PGHOST:$PGPORT/$PGDATABASE?ssl=1&sslmode=no-verify&sslrootcert=$FILE"