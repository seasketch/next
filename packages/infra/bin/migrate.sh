...
# Will run `git checkout master` and then the latest database migrations.
...
aws ecs execute-command --cluster $CLUSTER --task $TASK --container Default --command "/bin/sh -l /home/migrate.sh" --interactive
