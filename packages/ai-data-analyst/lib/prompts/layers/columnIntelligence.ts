...
export const columnIntelligencePrompt = `
...
export const columnIntelligenceSchema: JSONSchema4 = {
  ...
  properties: {
    ...
    best_group_by_column: {
      type: ["string", "null"],
      description:
        "Identify a column that should be used to group features for overlap analysis. Leave null if there is no good choice. If using a categorical presentation type, the group_by column should be consistent with the chosen_presentation_column. (Optional. Vector layers only)",
    },
    junk_columns: {
      type: "array",
      items: { type: "string" },
    ...
    chosen_presentation_column: {
      type: ["string", "null"],
      description:
        "If the chosen_presentation_type uses a data-driven style, this is the column to reference. Leave null for raster layers.",
    },
    palette: {
      type: ["string", "null"],
      description:
...
const validator = ajv.compile(columnIntelligenceSchema);

export const columnIntelligenceValidator = (data: any) => {
  const d = {
    ...data,
  };
